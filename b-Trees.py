# B - Trees
# 
# B-tree nodes ahve moe than two children
# A B-tree node may contain more than just a single element.
# Rule 1: The root can have as fwew as one element (or even no elements if it also has no children); every other node has at least the MINIMUM number of elements. 
...
# Rule 5: For any non-leaf node:
  # An element at index i is greater than all teh elements in the subtree number i of the node, and
